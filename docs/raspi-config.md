...
The variables are the values I would have selected from the curses UI. Locale options are the first field in the file `/usr/share/i18n/SUPPORTED`. Keyboard layout options are the directory names in `/usr/share/X11/xkb/symbols/`.